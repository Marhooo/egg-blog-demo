...
module.exports = {
  ...
  //微信用户加密信息解密
  wxUserDecryptData(sessionKey, data, ivv) {
    ...
    if (!verifyResult.verify) {
      ...
      if (tokenInfo) {
        ...
        if (!verify(tokenInfo.refresh_token).verify) {
          ...
          this.error(401, 10000, 'token身份认证失效,请重新登录');
        } else {
  ...
  async uploadImg() {
    ...
    // 生成文件名
    const filename =
      Date.now() + '' + Number.parseInt(Math.random() * 10000) + path.extname(stream.filename);
    // 生成文件夹
